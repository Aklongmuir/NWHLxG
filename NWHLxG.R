NWHLpbp1617 <- read_csv("nwhl_pbp_1617.csv")
NWHLpbp1718 <- read_csv("nwhl_pbp_1718.csv")
xgpbp <- bind_rows(NWHLpbp1617, NWHLpbp1718)
...
xGmodel <-glm (is_goal ~ poly(event_distance, 3, raw = TRUE) +
                 poly(event_angle, 3, raw = TRUE) + is_rebound + is_rush,
               data = Train_Fenwick_Data,
               family = binomial(link = 'logit'))

save(xGmodel, file = "xGmodelver2.rda")

testdata <- read_csv("nwhl_pbp_1617.csv")
...
#COOL RINK GRAPH THAT DOESNT  WORK
#avg_xG_by_coord$x_coord_1 <- round(avg_xG_by_coord$x_coord_1, digits=0)
#avg_xG_by_coord$y_coord_1 <- round(avg_xG_by_coord$y_coord_1, digits=0)

#ggplot(avg_xG_by_coord, aes(x_coord_1, y_coord_1, fill = avg_xG_by_coord$xg)) + geom_raster() +
...
#Pretty curve which keeps tell me im mediocre
